...
describe('add a task to list', () => {
  test("add a task 'Laundry' to the local storage and check if it exists on the local storage ", () => {
    ...
    expect((localStorage).getItem('todoList')).toBe(JSON.stringify([{ description: 'Laundry', complete: false, index: 0 }]));
  });

  test("add a task 'Cooking' to the local storage and check if it exists on the local storage ", () => {
    tasks.addBook('Cooking');
    expect((localStorage).getItem('todoList')).toBe(JSON.stringify([{ description: 'Laundry', complete: false, index: 0 }, { description: 'Cooking', complete: false, index: 1 }]));
  });
});
